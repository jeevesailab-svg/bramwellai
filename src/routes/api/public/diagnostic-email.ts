...
function renderReportEmail(row: ReportRow): string {
  ...
  const pathway = row.recommended_pathway_name ?? "your pathway";
  const price = row.recommended_price ?? "";
  ...
  return `<!doctype html><html><body style="margin:0;background:#0b0b0f;color:#f4f1ea;font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    <p style="font-size:12px;letter-spacing:.2em;text-transform:uppercase;color:#a59c8a;margin:0 0 12px;">Bramwell AI</p>
    <h1 style="font-size:22px;margin:0 0 20px;">Hi ${name}, here is your Readiness Score</h1>
    <div style="text-align:center;padding:24px;border:1px solid #2a2a33;border-radius:16px;background:#111118;">
      <div style="font-size:64px;font-weight:600;color:#d6b87a;line-height:1;">${score}<span style="font-size:22px;color:#a59c8a;font-weight:300;">/100</span></div>
      <p style="font-size:12px;letter-spacing:.2em;text-transform:uppercase;color:#a59c8a;margin:12px 0 0;">Communication Readiness</p>
    </div>
    ${type ? `<p style="margin:24px 0 8px;font-size:14px;color:#a59c8a;text-transform:uppercase;letter-spacing:.2em;">Your communication type</p><p style="margin:0;font-size:18px;color:#d6b87a;text-transform:capitalize;">The ${esc(type)}</p>` : ""}
    ${gapsHtml ? `<h2 style="font-size:16px;margin:28px 0 12px;">Your top gaps</h2><ul style="padding-left:18px;margin:0;color:#e6e1d4;font-size:14px;line-height:1.6;">${gapsHtml}</ul>` : ""}
    <h2 style="font-size:16px;margin:28px 0 8px;">Recommended next step</h2>
    <p style="margin:0 0 20px;color:#e6e1d4;font-size:14px;line-height:1.6;">${esc(pathway)}${price ? ` , ${esc(price)}` : ""}.</p>
    <p style="margin:24px 0 0;font-size:12px;color:#a59c8a;">Your full interactive report is on screen at bramwellai.lovable.app.</p>
  </div>
</body></html>`;
}
